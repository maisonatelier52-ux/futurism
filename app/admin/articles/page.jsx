...
import {
  getCategories, getArticles, saveArticle, deleteArticle, uploadImage,
} from "@/lib/categoriesArticlesApi";
import { ARTICLE_LAYOUT_OPTIONS, DEFAULT_SAGA_SO_FAR } from "@/lib/articleTemplateDefaults";
...
// ─── Saga So Far editor (Variation 1 only) ────────────────────────────────────

function SagaSoFarEditor({ items, onChange }) {
  const inp = "w-full border border-border rounded-lg px-3 py-2 text-[12.5px] text-ink-900 bg-surface-soft focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary/40 transition-colors";
  ...
  function updateItem(i, val) {
    onChange(items.map((it, idx) => (idx === i ? { ...it, text: val } : it)));
  }
    ...
    <div className="space-y-2">
      {items.map((item, i) => (
        <div key={item.id ?? i} className="flex items-center gap-2">
          <span className="text-[11px] font-bold text-ink-400 w-5 shrink-0">{String(i + 1).padStart(2, "0")}</span>
          <input type="text" value={item.text}
            onChange={(e) => updateItem(i, e.target.value)}
            placeholder="Timeline step…" className={inp} />
...
const BLOCK_STYLES = {
  paragraph:    "bg-blue-50 text-blue-600 border-blue-200",
...
const BLOCK_TYPES = [
  { type: "paragraph",   label: "Paragraph",   icon: AlignLeft,  cls: "bg-blue-600 hover:bg-blue-700" },
...
const EMPTY_FORM = {
  ...
  categoryId: "", newsType: "news", type: "normal",
  date: new Date().toISOString().split("T")[0],
  readTime: "", isPublished: true, imageAlt: "", keywords: "", tags: "",
  layoutVariation: "variation-4",
};
...
export default function ArticlesPage() {
  ...
  // NOTE: despite the name (kept to minimize diff), this now holds the
  // Cloudinary URL returned by uploadImage(), not a base64 string.
  ...
  // Variation 1 only: "The Saga So Far" numbered timeline
  const [sagaSoFar, setSagaSoFar] = useState([]);
  ...
  const loadAll = useCallback(async () => {
    ...
    const arts = await getArticles(selectedCat);
    setArticles(arts);
    setLoading(false);
  }, [selectedCat]);
  ...
  async function handleMainImg(e) {
    ...
    try {
      ...
      // then swap to the permanent Cloudinary URL once it resolves.
      const localPreview = URL.createObjectURL(f);
      setMainImgPreview(localPreview);
      const { url } = await uploadImage(f, "articles");
      setMainImgBase64(url); // holds the Cloudinary URL (see note on state declaration above)
      setMainImgPreview(url);
      if (formErrors.mainImage) setFormErrors((p) => ({ ...p, mainImage: "" }));
    } catch {
      setImageToast("Image upload failed. Please try again.");
      setMainImgPreview("");
    } finally {
  ...
  async function pickBlockImage(i, file) {
    ...
    setContentBlocks((p) =>
      p.map((b, idx) =>
        idx === i ? { ...b, imageFile: file, imagePreview: localPreview, src: b.src } : b
      )
    );
    try {
      ...
      setContentBlocks((p) =>
        p.map((b, idx) =>
          idx === i ? { ...b, imageFile: null, imagePreview: url, src: url } : b
        )
      );
  ...
  async function handleSubmit(evt) {
    ...
    setSaving(true);
    try {
      const cleanContent = contentBlocks.map(({ imageFile, ...rest }) => rest);
      const payload = {
        ...formData,
        mainImage: mainImgBase64 || (editingArticle?.mainImage ?? ""),
        content: cleanContent,
        keywords: formData.keywords.split(",").map((k) => k.trim()).filter(Boolean),
        tags: formData.tags.split(",").map((t) => t.trim()).filter(Boolean),
        ...(formData.layoutVariation === "variation-1" ? { sagaSoFar } : { sagaSoFar: [] }),
  ...
  function openModal(article = null) {
    if (article) {
      ...
      setFormData({
        ...
        layoutVariation:  article.layoutVariation || "variation-4",
      });
      setContentBlocks(
      ...
      setSagaSoFar(
        article.sagaSoFar?.length ? article.sagaSoFar : DEFAULT_SAGA_SO_FAR.map((s) => ({ ...s }))
      );
      ...
      setContentBlocks([]);
      setSagaSoFar(DEFAULT_SAGA_SO_FAR.map((s) => ({ ...s })));
      setMainImgPreview("");
      setMainImgBase64("");
    }
    setFormErrors({});
  ...
  async function filterCategory(catId) {
    ...
    setSearch("");
    setLoading(true);
    const arts = await getArticles(catId);
    setArticles(arts);
    setLoading(false);
  }
    ...
    <AdminShell title="Articles">
      <div className="p-4 lg:p-6 max-w-[1200px] mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          ...
          <button
            onClick={() => openModal()}
            className="flex items-center justify-center gap-2 px-4 py-2.5 bg-primary text-black rounded-lg hover:bg-primary/90 transition-all cursor-pointer font-medium text-sm w-full sm:w-auto"
          >
            ...
            <div className="bg-white rounded-2xl shadow-2xl overflow-hidden">
              ...
              <form onSubmit={handleSubmit} className="px-5 sm:px-6 pb-0 pt-5 space-y-6">
                ...
                <div className="space-y-4">
                  <SectionHeader icon={Newspaper} title="Layout Variation" />
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {ARTICLE_LAYOUT_OPTIONS.map((t) => (
                      <button key={t.value} type="button"
                        onClick={() => set({ layoutVariation: t.value })}
                        className={`text-left rounded-xl border p-3 transition-all cursor-pointer ${
                          formData.layoutVariation === t.value
                            ? "border-primary bg-primary-50 shadow-soft"
                            : "border-border bg-white hover:border-ink-300"
                        }`}
                      >
                        <div className="flex items-start gap-2">
                          <span className="text-lg shrink-0">{t.thumb}</span>
                          <div className="min-w-0">
                            <div className="flex items-center justify-between gap-2">
                              <span className="text-[12.5px] font-bold text-ink-900">{t.label}</span>
                              {formData.layoutVariation === t.value && (
                                <span className="text-[9px] font-bold uppercase text-primary shrink-0">✓</span>
                              )}
                            </div>
                            <p className="text-[11px] font-semibold text-ink-600 mt-0.5">{t.desc}</p>
                            <p className="text-[10px] text-ink-400 mt-1 leading-relaxed">{t.detail}</p>
                          </div>
                        </div>
                      </button>
                    ))}
                ...
                {/* ── Saga So Far (Variation 1 only) ── */}
                {formData.layoutVariation === "variation-1" && (
                  <div className="space-y-4">
                    <SectionHeader icon={List} title='"The Saga So Far" Timeline (Variation 1)' />
                    <p className="text-[11.5px] text-ink-400 -mt-2">
                      Shown in the left sidebar of Variation 1. The last step is highlighted as the current stage.
                    </p>
                    <SagaSoFarEditor items={sagaSoFar} onChange={setSagaSoFar} />
                  </div>
                )}
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    {mainImgPreview && (
                      <div className="relative shrink-0">
                        ...
                        <button type="button"
                          onClick={() => { setMainImgBase64(""); setMainImgPreview(""); }}
                ...
                <div className="sticky bottom-0 bg-white/95 backdrop-blur border-t border-border -mx-5 sm:-mx-6 px-5 sm:px-6 py-4 flex gap-3 rounded-b-2xl">
                  ...
                  <button type="submit" disabled={saving || uploadingMainImg}
                    className="flex-1 py-2.5 bg-primary text-black rounded-lg hover:bg-primary/90 transition-all font-semibold disabled:opacity-50 cursor-pointer flex items-center justify-center gap-2 text-[13px]">
                    <Save size={14} />
                    {uploadingMainImg ? "Uploading image…" : saving ? "Saving…" : editingArticle ? "Update Article" : "Save Article"}
                  </button>
                </div>
          ...
          <div className="flex justify-between items-start gap-3">
            <div className="flex-1 min-w-0">
              <div className="flex flex-wrap gap-2 mb-1.5">
                ...
                {article.layoutVariation && (
                  <span className="text-[11px] px-2 py-0.5 bg-indigo-100 text-indigo-700 rounded-full font-medium">
                    {ARTICLE_LAYOUT_OPTIONS.find((t) => t.value === article.layoutVariation)?.label || article.layoutVariation}
                  </span>
                )}
              </div>
