...
export function Field({ label, hint, children }) {
  return (
    <label className="block">
      {label && <span className="block text-[12px] font-medium text-ink-500 mb-1.5">{label}</span>}
      {children}
...
export function Input({ className = "", ...props }) {
  return (
    <input
      className={`w-full rounded-lg border border-border bg-surface-soft px-3 py-2 text-[13px] text-ink-900 focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary/40 ${className}`}
      {...props}
    />
  );
}
